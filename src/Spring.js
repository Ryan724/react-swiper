...
export const TransitionSpring = React.createClass({
  ...
  componentWillReceiveProps({currVelocity: currVelocityFunc, currValue: currValueFunc}) {
    const {currVelocity, currValue} = this.state;

    if (currVelocityFunc || currValueFunc) {
      if (currVelocityFunc) this.currVelocity = currVelocityFunc(currVelocity);
      // if (currValueFunc) this.currValue = currValueFunc(currValue);
